...
local function is_nonempty_string_array(v)
  ...
  for _, item in ipairs(v) do
    if not is_nonempty_string(item) then return false end
  end
  return true
end
...
local function check_object_keys(payload, allowed)
  ...
  for k in pairs(payload) do
    if not set[k] then return ("payload has unknown field: %s"):format(k) end
  end
end
...
rule("selection_changed", "event", { "instance_path" }, function(p)
  if p.instance_path == nil then return "payload.instance_path missing" end
  if type(p.instance_path) == "string" then
    if #p.instance_path == 0 then return "payload.instance_path must be non-empty" end
    return nil
  end
...
rule("signal_selected", "event", { "signal", "wave_scope" }, function(p)
  if not is_nonempty_string(p.signal) then return "payload.signal must be a non-empty string" end
  if not is_nonempty_string(p.wave_scope) then return "payload.wave_scope must be a non-empty string" end
end)

rule("cursor_time_changed", "event", { "t_fs" }, function(p)
  if not is_nonempty_string(p.t_fs) then return "payload.t_fs must be a non-empty string" end
...
rule("source_focused", "event", { "file", "line", "col" }, function(p)
  if not is_nonempty_string(p.file) then return "payload.file must be a non-empty string" end
  if not is_positive_int(p.line) then return "payload.line must be a positive integer" end
  if not is_positive_int(p.col) then return "payload.col must be a positive integer" end
end)
...
rule("open_source", "response", { "ok" }, function(p)
  if type(p.ok) ~= "boolean" then return "payload.ok must be a boolean" end
end)
...
rule("wave_add_variables", "response", { "ids" }, function(p)
  if type(p.ids) ~= "table" then return "payload.ids must be an array of non-negative integers" end
...
rule("view_pan_to", "request", { "instance_path" }, function(p)
  if not is_nonempty_string(p.instance_path) then return "payload.instance_path must be a non-empty string" end
end)
...
rule("resolve_signal_to_view", "response", { "instance_path", "port" }, function(p)
  ...
  if not is_nonempty_string(p.port) then return "payload.port must be a non-empty string" end
end)

rule("hello", "request", { "client", "version", "capabilities" }, function(p)
  local valid_origin = { view = true, wave = true, src = true, cli = true }
  if not valid_origin[p.client] then return "payload.client must be one of view|wave|src|cli" end
  if not is_nonempty_string(p.version) then return "payload.version must be a non-empty string" end
  if type(p.capabilities) ~= "table" then return "payload.capabilities must be an array" end
  local seen = {}
  for i, cap in ipairs(p.capabilities) do
    ...
    if seen[cap] then return ("payload.capabilities[%d] duplicates earlier item"):format(i - 1) end
    seen[cap] = true
  end
end)

rule("welcome", "response", { "server_version", "registered_clients" }, function(p)
  if not is_nonempty_string(p.server_version) then return "payload.server_version must be a non-empty string" end
  if type(p.registered_clients) ~= "table" then return "payload.registered_clients must be an array" end
  local valid_origin = { view = true, wave = true, src = true, cli = true }
  local seen = {}
  for i, c in ipairs(p.registered_clients) do
    ...
    if seen[c] then return ("payload.registered_clients[%d] duplicate"):format(i - 1) end
    seen[c] = true
  end
end)
...
RULES["bye"] = {
  event = function(payload)
    ...
    if type(payload) ~= "table" then return "payload must be null or empty object" end
    for _ in pairs(payload) do return "payload must be empty object" end
  end,
}

rule("error", "error", { "code", "message", "context" }, function(p)
  local codes = { unresolvable = true, not_connected = true, bad_request = true, protocol_mismatch = true }
  if not codes[p.code] then
    return "payload.code must be one of unresolvable|not_connected|bad_request|protocol_mismatch"
  end
  if not is_nonempty_string(p.message) then return "payload.message must be a non-empty string" end
...
function M.ensure_schema_loaded()
  if M._schema_path then return M._schema_path end
...
function M.validate(env)
  ...
  if type(env) ~= "table" then return "envelope must be a table" end
  local rules_for_type = RULES[env.type]
  if not rules_for_type then return nil end  -- unknown type: drop per §11
    ...
    vim.schedule(function()
      vim.notify(
        ("rtlbuddy: schema violation [%s/%s]: %s"):format(
          tostring(env and env.type), tostring(env and env.kind), err),
        vim.log.levels.WARN
      )
    end)
  end
